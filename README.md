...
首次使用接入统计的版本会先展示说明，默认开启，可随时在设置中关闭。只上传随机安装标识和应用版本，不上传学校、学号、账号或课程内容。

标识保存在 Android 不参与备份的目录内，覆盖升级会保留，重装或清除数据可能生成新标识。应用仅在前台使用时上报，每个北京时间自然日成功一次；调试构建和演示模式不计数。

管理员可查看累计设备、今日活跃和包含今天的近 30 天活跃。它们代表新版接入后成功上报的安装实例，不等于自然人数，也无法追溯旧版本。关闭统计会停止后续上报，已有记录不再更新。统计页使用 Cloudflare Access 邮箱验证码和服务端 JWT 验证限制访问，部署方式见 [统计服务说明](usage-stats-service/README.md)。
