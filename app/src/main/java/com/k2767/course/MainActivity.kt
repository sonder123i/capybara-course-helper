...
import com.k2767.course.utils.RecoveryPhase
import com.k2767.course.ui.system.SessionNoticeViewModel
...
import com.k2767.course.ui.system.PageDataViewModel
import com.k2767.course.ui.system.LocalPageDataState
import com.k2767.course.ui.system.NavScrollIntent
import com.k2767.course.ui.system.AppSymbolSpec
import com.k2767.course.ui.system.GlassOverlayHost
...
import com.k2767.course.activation.ActivationScreen
import com.k2767.course.manager.AppearanceSettingsManager
import com.k2767.course.manager.SmartSelector
import com.k2767.course.manager.UserManager
import com.k2767.course.ui.screen.OnboardingScreen
import com.k2767.course.ui.screen.SchoolAdaptationCompletionReminder
import com.k2767.course.ui.system.CapsuleNavigationBar
import com.k2767.course.ui.system.DialogHost
import com.k2767.course.ui.system.GlassToastHost
...
import com.k2767.course.ui.system.NoticeAnchorState
...
import com.k2767.course.update.rememberUpdateState
...
import com.k2767.course.manager.StartupPage
...
class MainActivity : FragmentActivity() {
    override fun attachBaseContext(newBase: android.content.Context) {
        super.attachBaseContext(com.k2767.course.manager.AppThemeCoordinator.wrapContext(newBase))
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        com.k2767.course.ui.theme.StartupLogoAnimation.install(this)
        ...
        if (BuildConfig.UI_PREVIEW) {
            userManager.startDemoSession(com.k2767.course.demo.DemoData.school())
        }
        ...
        setContent {
            CourseSelectorTheme {
                ...
                com.k2767.course.ui.screen.UsageNotice()
            }
        }
    }
...
fun MainScreen(fragmentActivity: FragmentActivity) {
    ...
    val surveyModel: com.k2767.course.survey.SurveyViewModel = viewModel()
    ...
    val surveyVisit by com.k2767.course.survey.SurveyVisitTracker.visit.collectAsState()
    val surveyUsagePreferences by com.k2767.course.usage.UsageStatsManager.preferences.collectAsState()
    ...
    val navigationMotion = com.k2767.course.ui.theme.rememberNavigationMotionState(selectedTab, currentAccountStorageKey, accessibility.reduceMotion)
    val reminderRequest = com.k2767.course.schedule.CourseReminderNavigation.requestedId
    LaunchedEffect(reminderRequest, currentAccountStorageKey) {
        if (reminderRequest != null) {
            if (com.k2767.course.schedule.ScheduleReminderScheduler.get(context).findById(reminderRequest) != null) selectedTab = 1
            else {
                com.k2767.course.ui.system.GlassToaster.show("这条课程提醒已失效或属于其他账号")
    ...
    val lensFreshness = remember {
        if (com.k2767.course.ui.system.glass.isGlassLensApplicable()) {
            com.k2767.course.ui.system.glass.GlassLensFreshness()
        } else {
            null
        }
    }
    ...
    DisposableEffect(fragmentActivity, session.token, session.expired, foreground) {
        if (foreground && !isDemoMode && UserManager.getInstance().currentSchool != null && !session.expired) {
            com.k2767.course.utils.CookieWatchdog.start(fragmentActivity)
        }
        onDispose {
            com.k2767.course.utils.CookieWatchdog.stop()
        }
    }
    ...
    GlassOverlayHost(modifier = Modifier.fillMaxSize()) {
        ...
        val navBarContentInset = com.k2767.course.ui.system.NavBarMetrics.contentInset()
        ...
        val appLensAnchor = if (wallpaperBackdrop != null) {
            com.k2767.course.ui.system.glass.rememberGlassLensRegion(
                tag = "app",
        ...
        CompositionLocalProvider(
            ...
            com.k2767.course.ui.system.glass.LocalGlassLensModalAnchor provides modalLensAnchor
        ) {
            Box(
            ...
            Column(modifier = Modifier.fillMaxSize()) {
                    Box(
                        ...
                        key(currentAccountStorageKey) {
                            ...
                            com.k2767.course.ui.theme.NavigationPages(navigationMotion,
                                modifier = Modifier.fillMaxSize().graphicsLayer {
                                    translationX = -6.dp.toPx() * dialogHostState.pageProgress
                                    val entry = com.k2767.course.ui.theme.StartupLogoAnimation.contentProgress
            ...
            CompositionLocalProvider(com.k2767.course.ui.theme.LocalNavigationMotion provides navigationMotion) {
            CapsuleNavigationBar(
                ...
                modifier = Modifier.align(Alignment.BottomCenter).graphicsLayer {
                    val entry = ((com.k2767.course.ui.theme.StartupLogoAnimation.contentProgress - 0.15f) / 0.85f).coerceIn(0f, 1f)
                    alpha = entry
            ...
            GlassToastHost(
                modifier = Modifier
                    ...
                    .padding(bottom = navBarContentInset + if (selectedTab == 2)
                        com.k2767.course.ui.system.TaskControlsReservedHeight + 12.dp else 12.dp)
            )
            ...
            if (showSurveyCenter) {
                key(currentAccountStorageKey) {
                    com.k2767.course.ui.system.GlassSubpage(onDismiss = { showSurveyCenter = false; initialSurveyId = null }) { close ->
                        com.k2767.course.ui.route.SurveyCenterRoute(surveyRepository, onBack = close, initialSurveyId = initialSurveyId)
                    }
                }
            }
            com.k2767.course.ui.screen.SurveyReminder(
            ...
            if (sessionNotice.token == session.token && sessionNotice.visible && isTokenExpired) {
                key(session.token) {
                    com.k2767.course.ui.system.SessionExpiryPrompt(
                    ...
                    confirmButton = {
                        SystemPrimaryButton(
                            ...
                            onClick = {
                                ...
                                try {
                                    val intent = android.content.Intent(android.content.Intent.ACTION_VIEW, android.net.Uri.parse("https://github.com/sonder123i/capybara-course-helper")).apply {
