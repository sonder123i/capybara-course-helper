package com.k2767.course.service
...
import com.k2767.course.utils.TeachingClassMatcher
...
class GrabService : Service() {
    ...
    companion object {
        ...
        // Action constants
        const val ACTION_START = "com.k2767.course.action.START_GRAB"
        const val ACTION_START_KEYWORD = "com.k2767.course.action.START_GRAB_KEYWORD"  // 关键词模式
        const val ACTION_START_QUEUE = "com.k2767.course.action.START_GRAB_QUEUE"      // 🔧 新增：直接队列模式
        const val ACTION_START_FUZZY_MATCH = "com.k2767.course.action.START_FUZZY_MATCH" // 🔧 模糊匹配模式
        const val ACTION_STOP = "com.k2767.course.action.STOP_GRAB"
        ...
        const val BROADCAST_UPDATE = "com.k2767.course.GRAB_UPDATE"
    ...
    private val cookieExpiredReceiver = object : android.content.BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            if (intent?.action == com.k2767.course.network.CourseApiClient.ACTION_COOKIE_EXPIRED) {
    ...
    override fun onCreate() {
        ...
        // 🔧 注册 Cookie 失效监听
        val filter = android.content.IntentFilter(com.k2767.course.network.CourseApiClient.ACTION_COOKIE_EXPIRED)
                                ...
                                if (!active.sessionState.isCurrent(expected) || !SessionRenewer.canRenew()) false
                                    ...
                                    SessionRenewer.request(expected) { result ->
                                        if (continuation.isActive) {
                                            if (result is com.k2767.course.utils.SessionRecoveryResult.Recovered && active.sessionState.isCurrent(result.token)) {
                                            ...
                                            } else if (result is com.k2767.course.utils.SessionRecoveryResult.Superseded) {
                                                continuation.cancel()
    ...
    private fun smartModeMatchFromAllCourses(school: SchoolConfig, task: GrabTaskContext) {
        ...
        CourseApiClient.getInstance().fetchAvailableCourses(school, buildCourseListBody(task.params, task.course), serviceAccountStorageKey, object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                try {
                    val courses = com.k2767.course.utils.CourseParser.parseCourseListFromJson(json)
    ...
    private fun checkCookieValidity(school: SchoolConfig, callback: (Boolean) -> Unit) {
        ...
        CourseApiClient.getInstance().validateCookie(school, serviceAccountStorageKey, object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                // 判断逻辑：尝试解析学生姓名
                val studentName = com.k2767.course.utils.CourseParser.parseStudentName(html)
                ...
                handler.post {
                    ...
                    if (isValid) {
                        ...
                        if (com.k2767.course.utils.SessionRenewer.canRenew()) {
                            broadcastLog("注意：登录状态已失效，正在自动续期")
                            com.k2767.course.utils.SessionRenewer.request(expected) { result ->
                                if (result is com.k2767.course.utils.SessionRecoveryResult.Recovered && sessions.isCurrent(result.token)) {
                                    broadcastLog("成功：登录状态已恢复，继续执行")
                                    callback(true)
                                } else if (result is com.k2767.course.utils.SessionRecoveryResult.NeedsLogin && sessions.isCurrent(expected)) {
                                    handleCookieInvalid()
                                    callback(false)
                                }
                            }
