...
import com.k2767.course.service.GrabService
import com.k2767.course.ui.screen.CourseListScreen
import com.k2767.course.ui.screen.GrabProScreen
import com.k2767.course.ui.screen.SelectedCoursesScreen
...
fun AcademicCourseListRoute(school: SchoolConfig) {
    ...
    if (showFilters) SystemDialog(onDismissRequest = { showFilters = false }, title = { Text("课程筛选") },
        ...
        Column(Modifier.fillMaxWidth().heightIn(max = 420.dp).verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            com.k2767.course.ui.screen.SchoolFormField("教师", draftFilter.teacher, { draftFilter = draftFilter.copy(teacher = it) })
            com.k2767.course.ui.screen.SchoolFormField("上课时间", draftFilter.time, { draftFilter = draftFilter.copy(time = it) })
            com.k2767.course.ui.screen.SchoolFormField("上课地点", draftFilter.location, { draftFilter = draftFilter.copy(location = it) })
            com.k2767.course.ui.screen.SchoolFormField("学分", draftFilter.credit, { draftFilter = draftFilter.copy(credit = it) })
...
fun AcademicGrabQueueRoute(school: SchoolConfig) {
    ...
    GrabProScreen(isRunning = running, successCount = success, failCount = failed, retryCount = retries,
        ...
        queueItemStatuses = itemStatuses.mapValues { (_, value) ->
            if (!running && value == "GRABBING") com.k2767.course.ui.screen.GrabQueueItemStatus.WAITING
            else runCatching { com.k2767.course.ui.screen.GrabQueueItemStatus.valueOf(value) }.getOrDefault(com.k2767.course.ui.screen.GrabQueueItemStatus.WAITING)
        },
    ...
    if (showManualAdd) SystemDialog(onDismissRequest = { showManualAdd = false }, title = { Text("添加课程到队列") },
        content = {
            Column(Modifier.verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                com.k2767.course.ui.screen.SchoolFormField(label = "课程名称", value = inputCourse, onValueChange = { inputCourse = it }, placeholder = "完整课程名称")
                if (school.academicSystem == AcademicSystem.ZF.id) {
                    com.k2767.course.ui.screen.SchoolFormField(label = "教学班（选填）", value = inputSection, onValueChange = { inputSection = it },
                        placeholder = "例如：篮球0003", helper = "按教学班名称匹配，留空则不限")
                }
                com.k2767.course.ui.screen.SchoolFormField(label = "教师（选填）", value = inputTeacher, onValueChange = { inputTeacher = it })
                com.k2767.course.ui.screen.SchoolFormField(label = "上课时间（选填）", value = inputTime, onValueChange = { inputTime = it })
            }
        }, confirmButton = {
