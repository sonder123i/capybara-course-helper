...
fun SelectedCoursesRoute(refreshRevision: Int = 0, onLoadingChange: (Boolean) -> Unit = {}) {
    ...
    val requests = remember(sessions) { com.k2767.course.manager.SessionRequestGate(sessions) }
    DisposableEffect(requests) { onDispose { requests.cancelAll() } }
    
    var courses by com.k2767.course.ui.system.rememberPageData<List<Course>>("legacy.selected") { emptyList() }
    var loadedRevision by com.k2767.course.ui.system.rememberPageData("legacy.selected.revision") { -1 }
    var isLoading by remember { mutableStateOf(false) }
    var isDropping by remember { mutableStateOf(false) }
    
    // 缓存从服务器获取的学年/学期参数（退课 API 需要）
    var xkxnm by com.k2767.course.ui.system.rememberPageData("legacy.selected.year") { "" }
    var xkxqm by com.k2767.course.ui.system.rememberPageData("legacy.selected.term") { "" }

    fun isCurrentAccount(ticket: com.k2767.course.manager.SessionRequestTicket): Boolean {
