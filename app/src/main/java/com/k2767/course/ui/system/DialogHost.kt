        ...
        key(dialog.handle.key) {
            ...
            PredictiveBackHandler(enabled = state.dialogs.lastOrNull() === dialog && visibility.targetState) { events ->
                try {
                    ...
                    scope.launch {
                        dialog.bottomSheet?.restore() ?: backProgress.animateTo(0f, com.k2767.course.ui.theme.MotionSpring.snappy())
                    }
                }
            }
            AnimatedVisibility(visibleState = visibility, enter = EnterTransition.None, exit = ExitTransition.None) {
                val progress = transition.animateFloat(transitionSpec = {
                    when {
                        reducedMotion -> snap()
                        page -> com.k2767.course.ui.theme.MotionProfile.hierarchySpring()
                        targetState != EnterExitState.Visible -> tween(com.k2767.course.ui.theme.MotionProfile.SheetExitMillis)
                        bottom && dialog.bottomSheet != null -> tween(220,
                            easing = com.k2767.course.ui.theme.MotionEasing.FastOutSlowIn)
                        bottom -> com.k2767.course.ui.theme.MotionProfile.sheetSpring()
                ...
                Box(
                    ...
                    Box(
                        Modifier.graphicsLayer {
                                ...
                                translationX = (if (page) with(density) { com.k2767.course.ui.theme.MotionProfile.HierarchyEnterDp.dp.toPx() } *
                                    (1f - presence + back) else 0f) - with(density) { com.k2767.course.ui.theme.MotionProfile.HierarchyBehindDp.dp.toPx() } * above
