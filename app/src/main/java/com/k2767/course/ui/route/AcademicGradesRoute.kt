...
fun AcademicGradesRoute(school: SchoolConfig) {
    ...
    LaunchedEffect(account, revision, session.token) {
        ...
        try {
            ...
            if ((e as? AcademicException)?.status == AcademicStatus.SESSION_EXPIRED)
                com.k2767.course.network.CourseApiClient.getInstance().notifyCookieExpired(expected)
        }
    ...
    com.k2767.course.ui.system.ReportPageContent(report.grades.isNotEmpty() || exams.isNotEmpty())
