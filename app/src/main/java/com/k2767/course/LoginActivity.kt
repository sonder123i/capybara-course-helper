...
import com.k2767.course.ui.screen.LoginScreen
...
import com.k2767.course.login.PasswordLoginGateway
import com.k2767.course.login.PasswordLoginGatewayFactory
import com.k2767.course.academic.AcademicGatewayFactory
import com.k2767.course.academic.AcademicPasswordLoginGateway
import com.k2767.course.AcademicWebViewActivity
        ...
        lifecycleScope.launch {
            if (!BuildConfig.UI_PREVIEW) try {
                withContext(Dispatchers.IO) {
                    com.k2767.course.activation.ActivationManager.checkActivation(this@LoginActivity)
                }
                Log.d(TAG, "已同步云端配置")
            } catch (e: Exception) {
            ...
            CourseSelectorTheme {
                ...
                if (showSchoolAdaptation) {
                    ...
                    onConfirmBinding = {
                        ...
                        val recorded = com.k2767.course.manager.StudentLimitManager.recordStudent(
    ...
    private fun handleLogin(cookieStr: String) {
        ...
        lifecycleScope.launch {
            try {
                ...
                Log.d(TAG, "激活配置已同步，max_students=${com.k2767.course.activation.ActivationManager.getMaxStudents(this@LoginActivity)}")
            } catch (e: Exception) {
        ...
        if (AcademicGatewayFactory.supports(currentSchool)) {
            ...
            validationJob = lifecycleScope.launch {
                ...
                result.onSuccess { identity ->
                    if (identity.status != com.k2767.course.academic.AcademicStatus.SUCCESS) {
                        isLoading = false
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                runOnUiThread {
                    ...
                    if (success) {
                        ...
                        val maxStudents = com.k2767.course.activation.ActivationManager.getMaxStudents(this@LoginActivity)
                        val bindingCheck = com.k2767.course.manager.StudentLimitManager.checkCanUseStudent(
                            context = this@LoginActivity,
    ...
    private fun finishAcademicLogin(currentSchool: SchoolConfig, cookieStr: String, parsedName: String, parsedId: String) {
        ...
        runOnUiThread {
            ...
            if (bindingCheck.alreadyBound) {
                ...
                bindingMaxStudents = com.k2767.course.activation.ActivationManager.getMaxStudents(this)
