...
fun AcademicSupportDialog(currentSystem: String?, onDismiss: () -> Unit) {
    SystemDialog(onDismissRequest = onDismiss, title = { Text("四类教务支持与限制") },
        ...
        Column(Modifier.fillMaxWidth().heightIn(max = 480.dp).verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(14.dp)) {
            ...
            AcademicCapabilities.systems.forEach { support ->
                SystemDivider()
            ...
            Text("容量、成绩分项和历史学期以学校公布内容为准。旧正方没有历年汇总入口时，逐学期查询学校开放的成绩；未能完整读取时会提示错误。定制页面尚未适配与学校未开放权限会分别提示。",
                style = MaterialTheme.typography.bodySmall)
        }
    }
}
